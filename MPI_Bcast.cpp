...
void calculate_band_mean(const char *filename, int rank, int size) {
    ...
    // 准备一个足够大的向量来存储所有波段的平均值
    std::vector<double> all_band_means(band_count, 0.0);

    // 使用 MPI_Reduce 将所有进程的结果汇总到根进程
    MPI_Reduce(band_means.data(), all_band_means.data(), band_count, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);

    // 计算每个波段的最终平均值（如果波段被多个进程处理）
    if (rank == 0) {
        for (int i = 0; i < band_count; ++i) {
            all_band_means[i] /= size; // 假设每个波段被所有进程处理了一次
        }
    }

    // 广播所有波段平均值给所有进程
    MPI_Bcast(all_band_means.data(), band_count, MPI_DOUBLE, 0, MPI_COMM_WORLD);
...
int main(int argc, char **argv) {
    ...
    if (rank == 0) {
        ...
        std::cout << "MPI Method: MPI_Reduce and MPI_Bcast" << std::endl;
